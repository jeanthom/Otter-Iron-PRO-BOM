...
class Part < Struct.new(:value, :footprint, :arrowPN, :lcscPN, :qty)
	def initialize(value, footprint, arrowPN, lcscPN, qty = 0); super end
end
...
csvFiles.each do |file|
	CSV.foreach(file, headers: true, :header_converters=> lambda {|f| f.strip}) do |row|
		unless parts.any? { |part| (part[:lcscPN] == row["LCSC"]) && (part[:arrowPN] == row["Arrow"]) }
			parts << Part.new(row["Value"], row["Footprint"], row["Arrow"], row["LCSC"], 1)
		else
...
puts "| Value | Qty | Footprint | Arrow P/N | LCSC P/N |"
puts "|-------|-----|-----------|-----------|----------|"
parts.each do |part|
	puts "| #{part[:value]} | #{part[:qty]} | #{part[:footprint]} | #{part[:arrowPN]} | #{part[:lcscPN]} |"
end
